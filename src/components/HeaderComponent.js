...
const LoginForm = (props) => {
  ...
  return (
    <Modal
      isOpen={props.modal["login"]}
      toggle={() => props.toggleModal("login")}
    >
          ...
          <FormGroup>
            <Button
              ...
              onClick={() => {
                ...
                props.toggleModal("forgetPassword");
              }}
            >
              Forget Password
            </Button>
...
const ForgetPassword = (props) => {
  ...
  const handleforgetPassword = () => {
    props.forgetPassword({ email: email }).then((res) => {
      if (res.status) alert(res.status);
      else if (typeof res == "string") alert(res);
      else if (typeof res == "object" && res.err !== undefined) alert(res.err);
  ...
  return (
    <Modal
      isOpen={props.modal["forgetPassword"]}
      toggle={() => props.toggleModal("forgetPassword")}
    >
        ...
        >
          ...
          <FormGroup>
            ...
            <div className="text-dark">
              *An email will be sent contaning OTP to change Password.Check spam
              if not found.
            </div>
          </FormGroup>
        </LocalForm>
...
const ChangePassword = (props) => {
  ...
  const handlechangePassword = (email) => {
    props.changePassword({ email: email }).then((res) => {
      if (res.status) alert(res.status);
      else if (typeof res == "string") alert(res);
      else if (typeof res == "object" && res.err !== undefined) alert(res.err);
      else alert("Some error occured.\nPlease try again later");
  ...
  return (
    <Modal
      isOpen={props.modal["changePassword"]}
      toggle={() => props.toggleModal("changePassword")}
    >
      <ModalHeader>Forget Password</ModalHeader>
      <ModalBody>
        <LocalForm
...
const VerifyUser = (props) => {
  const handleVerifyUser = (value) => {
    ...
    props.verifyUser(value).then((res) => {
      if (res && res.status) {
        alert(res.status);
      } else if (typeof res == "string") alert(res);
      else if (typeof res == "object" && res.err !== undefined) alert(res.err);
  ...
  return (
    <Modal
      isOpen={props.modal["verifyUser"]}
      toggle={() => props.toggleModal("verifyUser")}
    >
      <ModalHeader>Verify Account</ModalHeader>
      <ModalBody>
        <LocalForm
          onSubmit={(val, e) => {
            handleVerifyUser(val);
            e.preventDefault();
          }}
        >
          <Row className="form-group">
          ...
          <Row className="form-group">
            <Col>
              <Button
                ...
                onClick={() => props.toggleModal("resendOTP")}
              >
                Resend OTP
              </Button>
              &nbsp;
...
const ResendOTP = (props) => {
  const handleresendOTP = (value) => {
    props.resendOTP(value).then((res) => {
      if (res && res.status) alert(res.status);
      else if (typeof res == "string") alert(res);
      else if (typeof res == "object" && res.err !== undefined) alert(res.err);
      else alert("Some error occured.\nPlease try again later");
    });
  ...
  return (
    <Modal
      isOpen={props.modal["resendOTP"]}
      toggle={() => props.toggleModal("resendOTP")}
    >
      <ModalHeader>Resend OTP</ModalHeader>
      <ModalBody>
        <LocalForm
          onSubmit={(val, e) => {
            props.toggleModal("resendOTP");
            handleresendOTP(val);
            e.preventDefault();
          }}
        >
          ...
          <FormGroup>
            ...
            <div className="text-dark">
              *An email will be sent contaning OTP to validate account.Check
              spam if not found.
            </div>
          </FormGroup>
        </LocalForm>
...
const RegisterForm = (props) => {
  ...
  return (
    <Modal
      isOpen={props.modal["register"]}
      toggle={() => props.toggleModal("register")}
    >
            ...
            <Col>
              ...
              <Control.text
                ...
                asyncValidators={{
                  availabl: (val, done) => {
                    setisloading(true);
                    props.availableUName(val).then((response) => {
                      setisloading(false);
                      if (!response.available) done(false);
                      else done(true);
                    });
                  },
                }}
    ...
    return (
      <Fragment>
        ...
        <NavItem>
          <Button
            color="primary"
            onClick={() => props.toggleModal("verifyUser")}
          >
...
class Header extends Component {
  ...
  render() {
    const img_name = ["bg1.jpg", "bg4.jpg", "bg8.jpg"];
    const CustomImg = img_name.map((img) => {
      return this.CustomCarouselItem(img);
    });
    return (
      <header>
              ...
              <Nav navbar>
                ...
                <NavItem>
                  <NavLink className="nav-link" to="/booking">
                    <span className="fa fa-cutlery fa-lg"></span> Book a table
                  </NavLink>
                </NavItem>
              </Nav>
