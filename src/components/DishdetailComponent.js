...
function CommentForm({postComment,dishId,open,setmodal,handleCmtForm}){
    var [isCmtFormOpen,updateCmtFormOpen]=useState(false)
    const toggleCmtFormOpen=()=>{updateCmtFormOpen(!isCmtFormOpen);}
    return(
        <div>
            <Modal isOpen={open} toggle={setmodal}>
                <ModalHeader>Submit Comments</ModalHeader>
                <ModalBody>
                    <LocalForm onSubmit={(values,e)=>{toggleCmtFormOpen();handleCmtForm(values,postComment,dishId);e.preventDefault();
                    }}>
                        ...
                        <Row className='form-group'>
                            ...
                            <Col md={10}>
                                ...
                                placeholder='Comments' className='form-control' required
                                />
                            </Col>
                        </Row>
                        <Row>
...
function RenderComment({cmts,postComments,dishId,username,commentDelete,commentEdit}){
    ...
    var [isCmtFormOpen,updateCmtFormOpen]=useState(false)
    const toggleCmtFormOpen=()=>{updateCmtFormOpen(!isCmtFormOpen);}
    const newComment=(values,postComments,dishId)=>{
    ...
    const comments=cmts.map((comment)=>{
        ...
        const CmtsOptions=()=>{
            ...
            const handleEditCmts=(values)=>{
                commentEdit(comment._id,dishId,values)
            }
            ...
            if(username && username===comment.author.username)
                return(
                    <Fragment>
                        ...
                        <CommentForm open={editCmts} setmodal={()=>seteditCmts(!editCmts)} postComment={postComments} dishId={dishId} handleCmtForm={handleEditCmts}/>             
                    </Fragment>
                )
            else
    ...
    return(
        <div>
            <Card>
                ...
                <CommentForm open={isCmtFormOpen} setmodal={toggleCmtFormOpen} postComment={postComments} dishId={dishId} handleCmtForm={newComment}/>
                <Paginate/>
            </Card>
        </div>
    )
